...
from Apps.liter_control.models import Provedor,Ruta,RegisterDay
...
class Indexview(View):
    ...
    def get(self, request):
        ...
        total_litros_leche_mes_actual = RegisterDay.objects.filter(day__month=mes_actual, day__year=año_actual).aggregate(total_litros=Sum('liter'))['total_litros']
        litros_por_mes = RegisterDay.objects.annotate(
            mes=TruncMonth('day')
        ).values('mes').annotate(
            nombre_mes=ExtractMonth('mes'),
        ...
        return render(request, self.template_name,{'total_provedores': total_provedores,
