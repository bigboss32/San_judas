...
class ObtenerProvedores(View):
    ...
    def get(self, request):
        ...
        litros_por_proveedor = RegisterDay.objects.filter(
            day__month=fecha_filtro.month,
            day__year=fecha_filtro.year
        ).values('provedor__first_name',
                'provedor__second_name',
                'provedor__last_name',
                'provedor__second_last_name',
                'provedor__cedula',
                'ruta__name_route'
                ).annotate(total_litros=Sum('liter'),
                             total_adelantos=Sum('adelantos'),
                             value_total_adelantos=Sum('value_total_adelantos'))
        total_provedores = Provedor.objects.count()
        total_litros_en_toda_la_consulta = litros_por_proveedor.aggregate(total_litros_en_toda_la_consulta=Sum('total_litros'))['total_litros_en_toda_la_consulta']
        total_adelantos_en_toda_la_consulta = litros_por_proveedor.aggregate(total_adelantos_en_toda_la_consulta=Sum('total_adelantos'))['total_adelantos_en_toda_la_consulta']
        total_total_en_toda_la_consulta = litros_por_proveedor.aggregate(total_total_en_toda_la_consulta=Sum('value_total_adelantos'))['total_total_en_toda_la_consulta']
        return render(request, self.template_name,{"litros_por_proveedor":litros_por_proveedor,"total_provedores":total_provedores,
